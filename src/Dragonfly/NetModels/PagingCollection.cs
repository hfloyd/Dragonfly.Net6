...
public class PagingCollection<T> : IEnumerable<T>
{
	...
	{
		return new PagingCollection<T>(collection, pageSize).GetPage(pageNumber, true).Collection;
	}
	...
	{
		return new PagingCollection<T>(collection, DefaultPageSize).GetPage(pageNumber, true).Collection;
	}
