...
#pragma warning disable 1591
			...
			string PagerQueryStringKey = "p")
		{
			...
			this.CurrentPageNum = ThisPageNum;
			this.TotalPages = TotalNumPages;
			this.QueryStringKey = PagerQueryStringKey;


			//Make sure the base url is clean of any existing page query string keys
			var cleanBaseUrl = Urls.RemoveQueryStringKeyFromUrl(PagerBaseUrl, QueryStringKey);

			//Set up base url with query string key and = sign (no value)
			if (cleanBaseUrl.Contains("?") && !cleanBaseUrl.EndsWith("?"))
			{
				//Contained somewhere in the middle, so there is existing query string data, append page info
				BaseUrlWithQS = $"{cleanBaseUrl.EnsureEndsWith('&')}{PagerQueryStringKey}=";
			}
			else
			{
				BaseUrlWithQS = $"{cleanBaseUrl.EnsureEndsWith('?')}{PagerQueryStringKey}=";
			...
			FirstUrl = cleanBaseUrl;
			LastUrl = $"{BaseUrlWithQS}{TotalPages}"; //use the last page number
